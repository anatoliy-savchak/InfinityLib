    ...
    public class BifFileBinaryReader : IBifFileReader
    {
        ...
        public BifFile Read(Stream s, List<KeyBifResource2> resources, List<IEFileType> fileTypes)
        {
            ...
            try
            {
                ...
                List<KeyBifResource2> resources2 = new List<KeyBifResource2>();
                resources2.AddRange(resources);
                int ix = 0;
                foreach (var f in fileStructs)
                {
                    if (resources2.Count == 0) break;
                            ...
                            switch ((IEFileType)f.resourceType)
                            {
                                case IEFileType.Are:
                                    try
                                    {
                                        ...
                                        resource = resources.Where(a => a.resourceLocator == f.resourceLocator).SingleOrDefault();
                                        //var ress = resources.Where(a => a.NonTileSetIndex == (f.resourceLocator & 0xFFF)).ToList();
                                        //resource = resources.Where(a => a.NonTileSetIndex == (f.resourceLocator & 0xFFF) && (short)a.ResourceType == f.resourceType).SingleOrDefault();
