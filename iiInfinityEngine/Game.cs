...
namespace iiInfinityEngine.Core
{
    public class Game
    {
        ...
        public void LoadResouce(string resKey, IEFileType resType)
        {
            var bifList = new List<KeyBifEntry2>();
            var relevantResources = key.Resources.Where(a => a.ResourceType == resType && a.ResourceName.ToLowerInvariant() == resKey).ToList();
            if (relevantResources.Count == 0) return;
            ...
            var fileTypes = new List<IEFileType>() { resType };
